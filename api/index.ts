...
app.post('/api/roast', async (req, res) => {
  ...
  try {
    ...
    if (isVercel) {
      ...
      console.log('Launching Puppeteer locally/in-container...');
      try {
        browser = await puppeteer.launch({
        ...
        console.error('Puppeteer launch error locally, trying fallback...', launchError.message);
        const paths = ['/usr/bin/google-chrome', '/usr/bin/chromium-browser', '/usr/bin/chromium'];
        for (const p of paths) {
          if (fs.existsSync(p)) {
            console.log(`Found chromium at ${p}, trying to launch...`);
            try {
              browser = await puppeteer.launch({
                executablePath: p,
                args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage'],
              ...
              console.log('Puppeteer launched successfully with fallback path');
              break;
            } catch (e: any) {
              console.error(`Failed to launch with ${p}:`, e.message);
            }
          }
        }
  ...
  if (!isVercel) {
    if (process.env.NODE_ENV !== 'production') {
      ...
      try {
        ...
        console.error('Failed to start Vite server:', viteError.message);
      }
    } else {
      // Production mode
    ...
    // ALWAYS listen on port 3000 in AI Studio / Cloud Run
    app.listen(Number(PORT), '0.0.0.0', () => {
      console.log(`Server running on http://0.0.0.0:${PORT} (NODE_ENV: ${process.env.NODE_ENV})`);
    });
  } else {
    // On Vercel, we still might want to log something
